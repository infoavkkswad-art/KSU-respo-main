...
# ============================================================
# PAYMENT DISMISSED / ABANDONED
# ============================================================

@router.post(
    "/orders/payment-dismissed",
)
async def payment_dismissed(
    request: Request,
    db: AsyncIOMotorDatabase = Depends(
        get_database
    ),
    ...
    """
    Called when the Razorpay checkout modal is closed.

    The browser dismissal alone is NOT trusted. Razorpay is
    checked first so a payment that was actually captured is
    never incorrectly marked cancelled.
    """
    ...
    if not razorpay_order_id:
        raise HTTPException(
            status_code=400,
            detail="razorpayOrderId is required.",
        )
    ...
    try:
        gateway_order = razorpay_client.order.fetch(
            razorpay_order_id
        )

        assert_razorpay_amount_matches_order(
            gateway_order.get("amount"),
            get_order_amount_in_paise(order),
        )

        payments_response = (
            razorpay_client.order.payments(
                razorpay_order_id
            )
        )

        payments = (
            payments_response.get("items", [])
            if isinstance(
                payments_response,
                dict,
            )
        ...
        print(
            "Razorpay dismissal check failed:",
            f"{type(e).__name__}: {str(e)}",
        )
    ...
    if processing_payment:
        return {
            "success": True,
            "paymentStatus": processing_payment.get(
                "status",
                "pending",
            ),
            "status": order.get(
    ...
    cancelled_order = await db.orders.find_one_and_update(
        {
            "_id": order["_id"],
            "paymentStatus": {
                "$in": [
    ...
    if not cancelled_order:
        cancelled_order = await db.orders.find_one(
            {"_id": order["_id"]}
        )

    customer = cancelled_order.get("customer", {})
    ...
    sheet_payload = {
        "type": "order",
        "data": {
            "orderId": cancelled_order.get(
                "orderId",
                "",
            ),
            "createdAt": serialize_datetime(
                cancelled_order.get("createdAt")
            ),
            ...
            "pincode": customer.get(
                "pincode",
                "",
            ),
            "items": cancelled_order.get(
                "items",
                [],
            ),
            "subtotal": cancelled_order.get(
                "subtotal",
                0,
            ),
            "shipping": cancelled_order.get(
                "shipping",
                0,
            ),
            "total": cancelled_order.get(
                "total",
                0,
            ),
            "fulfillmentType": cancelled_order.get(
                "fulfillmentType",
                "",
            ),
            "pricingMode": cancelled_order.get(
                "pricingMode",
                "",
            ),
            "shippingRequired": cancelled_order.get(
                "shippingRequired",
                False,
            ),
            "fulfillmentStatus": cancelled_order.get(
                "fulfillmentStatus",
                "AWAITING_PAYMENT",
            ),
            "currency": "INR",
            ...
            "razorpayPaymentId": cancelled_order.get(
                "razorpayPaymentId",
                "",
            ),
            "razorpaySignatureVerified": False,
            "lastPaymentEvent": "checkout_dismissed",
    ...
    try:
        ...
        print(
            "Google Sheets cancelled-order sync failed:",
            f"{type(e).__name__}: {str(e)}",
        )
...
async def razorpay_webhook(
    ...
    if event_type == "payment.failed":
        ...
        failed_order = await db.orders.find_one_and_update(
            ...
            },
            {
                "$set": {
                    ...
                    "fulfillmentStatus": "AWAITING_PAYMENT",
                    "paymentFailureReason": str(failure_reason),
        ...
        if not failed_order:
            ...
            if existing and existing.get("paymentStatus") == "paid":
                return {
                    ...
                    "event": event_type,
                    "paymentStatus": "paid",
                    "orderStatus": existing.get(
                        "status",
                        "confirmed",
                    ),
                }

            if existing and existing.get("paymentStatus") == "failed":
                return {
                    "status": "ok",
                    "event": event_type,
                    "paymentStatus": "failed",
        ...
        payment_record = extract_payment_sheet_data(
            failed_order,
            payment_entity,
            signature_verified=True,
            event_id=event_id or "",
        ...
        payment_record["paymentStatus"] = "failed"
        payment_record["failureReason"] = str(failure_reason)

        await db.orders.update_one(
            {"_id": failed_order["_id"]},
            {
                "$set": {
                    "paymentDetails": payment_record,
                    "paymentFailureReason": str(failure_reason),
                }
            },
        )

        customer = failed_order.get("customer", {})
        ...
        sheet_payload = {
            "type": "order",
            "data": {
                "orderId": failed_order.get("orderId", ""),
                "createdAt": serialize_datetime(
                    failed_order.get("createdAt")
                ),
                ...
                "items": failed_order.get("items", []),
                "subtotal": failed_order.get("subtotal", 0),
                "shipping": failed_order.get("shipping", 0),
                "total": failed_order.get("total", 0),
                "fulfillmentType": failed_order.get(
                    "fulfillmentType",
                    "",
                ),
                "pricingMode": failed_order.get(
                    "pricingMode",
                    "",
                ),
                "shippingRequired": failed_order.get(
                    "shippingRequired",
                    False,
                ),
                "fulfillmentStatus": failed_order.get(
                    "fulfillmentStatus",
                    "AWAITING_PAYMENT",
                ),
                "currency": "INR",
                ...
                "paymentMethod": payment_entity.get(
                    "method",
                    "",
                ),
                "razorpayOrderId": razorpay_order_id,
                ...
                "lastWebhookEvent": event_type,
                "lastWebhookAt": received_at,
                "createdSource": "razorpay_webhook",
            },
        }
        ...
        try:
            ...
            print(
                "Google Sheets failed-order sync failed:",
                f"{type(e).__name__}: {str(e)}",
            )
